...
import { browseFilesystemFolders, selectSystemFolder } from '../data/workspaceApi';
...
export default function WorkspacePathField({
  ...
  const [selectingSystemFolder, setSelectingSystemFolder] = useState(false);
  ...
  const handleBrowseClick = useCallback(async () => {
    if (disabled || selectingSystemFolder) {
      return;
    }

    setSelectingSystemFolder(true);

    try {
      const selectedPath = await selectSystemFolder(value.trim() || '~');
      if (selectedPath) {
        handleFolderSelected(selectedPath, workspaceType === 'existing');
      }
    } catch (error) {
      console.error('Failed to open system folder picker:', error);
      setShowFolderBrowser(true);
    } finally {
      setSelectingSystemFolder(false);
    }
  }, [disabled, handleFolderSelected, selectingSystemFolder, value, workspaceType]);
    ...
    <>
      <div className="relative flex gap-2">
        ...
        <Button
          ...
          disabled={disabled || selectingSystemFolder}
        >
          {selectingSystemFolder ? (
